...
export default function ReceiptAddForm({ id, fundDt, funderDt, companyProfileData , session }) {
  ...
  funderDt?.filter(item => item.funderName === formik.values?.funder).map(item => formik.setValues({ user: session?.createdBy, recepitDate: formik.values.recepitDate, fullName: item.funderName || '', contactPerson: item.contactPerson || '', contactNumber: item.contactNumber || '', email: item.email || '', pan: item.pan || '', addressLine1: item.addressLine1 || '', addressLine2: item.addressLine2 || '', country: item.country || '', state: item.state || '', pinCode: item.pinCode || '' }))
